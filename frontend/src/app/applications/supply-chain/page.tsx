...
import { ArrowRight, Check, ChevronRight, Package, Truck, Warehouse, ShoppingCart, BarChart3, AlertCircle } from 'lucide-react';
      ...
      <section className="pt-40 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12">
          <div>
            ...
            <h1 className="text-5xl sm:text-6xl font-bold text-white mb-6 uppercase font-serif">Supply Chain</h1>
            <p className="text-lg text-slate-400 mb-8">End-to-end inventory and procurement control. Manage your entire supply chain from procurement to delivery.</p>
            <div className="flex flex-wrap gap-3">
              {['Inventory', 'Procurement', 'Vendors', 'Warehousing'].map((tag, i) => (<span key={i} className="px-3 py-1 bg-slate-800 text-slate-300 text-sm rounded-full border border-slate-700">{tag}</span>))}
            </div>
          </div>
          <div className="bg-gradient-to-br from-blue-600 to-orange-500 rounded-2xl p-8 text-white">
            ...
            <p className="text-blue-100 mb-6">Complete visibility from procurement to delivery.</p>
            <ul className="space-y-3">
              {['Real-time inventory', 'Vendor management', 'Purchase orders', 'Stock alerts'].map((b, i) => (<li key={i} className="flex items-center gap-2"><Check className="w-5 h-5 text-blue-200" />{b}</li>))}
            </ul>
          </div>
        </div>
      </section>
